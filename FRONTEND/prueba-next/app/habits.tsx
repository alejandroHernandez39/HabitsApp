...
import {markHabitDoneThunk, fetchAddHabitThunk} from "@/features/habit/habitSlice";
...
const handleMarkHabitDone = (habitId: string, dispatch: AppDispatch, token:string) => {
    dispatch(markHabitDoneThunk({habitId, token}));
    if(token){
                        ...
                        <li className="flex items-center justify-between mt-2" key={habit._id}>
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <button className="px-2 py-1 text-sm text-white bg-blue-500 rounded" 
                                    onClick={() => handleMarkHabitDone(habit._id, dispatch, user ? user.toString() : '')}>
